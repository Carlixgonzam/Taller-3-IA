"""
Automated tests for propositional CNF transform.

Run with: pytest tests/test_cnf.py -v
"""

from src.logic.propositional.ast import And, Atom, Iff, Implies, Not, Or, evaluate, get_atoms
from src.logic.propositional.cnf import (
...
def _is_equivalent(f1, f2):
    """Check whether two formulas are logically equivalent."""
...
class TestPushNegationInward:
    ...
    def test_no_negation(self):
        """And(p, q) with no negation above does not change."""
        formula = And(Atom("p"), Atom("q"))
        result = push_negation_inward(formula)
        assert result == formula
...
class TestDistributeOrOverAnd:
    ...
    def test_no_distribution_needed(self):
        """Or(p, q) is already a clause."""
        formula = Or(Atom("p"), Atom("q"))
        result = distribute_or_over_and(formula)
        assert _is_equivalent(formula, result)

    def test_and_stays(self):
        """And(p, q) is already in CNF."""
        formula = And(Atom("p"), Atom("q"))
        result = distribute_or_over_and(formula)
        assert result == formula

    def test_nested_distribution(self):
        """Or(p, And(q, And(r, s))) requires multiple distributions."""
    ...
    def test_or_with_multiple_and_children(self):
        """Or(And(a,b), c, And(d,e)) has TWO And children.

        The algorithm only detects the FIRST And (and_index=0) on each call,
        then recursion handles the remaining And in 'others'.

        Manual check (partial truth table) for (a∧b) ∨ c ∨ (d∧e):
        Expected CNF (after flatten): (a∨c∨d) ∧ (a∨c∨e) ∧ (b∨c∨d) ∧ (b∨c∨e)
...
class TestFlatten:
    ...
    def test_single_element_after_flatten(self):
        """And(And(a, b)) flattens to And(a, b)."""
...
class TestToCNF:
    ...
    def test_nested_biconditional_chain(self):
        """Iff(p, Iff(q, r)) — nested biconditional with 3 variables."""
        formula = Iff(Atom("p"), Iff(Atom("q"), Atom("r")))
        cnf = to_cnf(formula)
        assert _is_equivalent(formula, cnf)
        assert _is_cnf(cnf)
    ...
    def test_four_variables_complex(self):
        """(p → q) ∧ (q → r) ∧ (r → s) — chain of implications."""
        formula = And(
            Implies(Atom("p"), Atom("q")),
            Implies(Atom("q"), Atom("r")),
    ...
    def test_double_negation_inside_iff(self):
        """Iff(Not(Not(p)), q) — double negation inside a biconditional."""
    ...
    def test_or_of_three_conjunctions(self):
        """Or(And(a,b), And(c,d), And(e,f)) — distribution with 3 conjunctions."""
        formula = Or(
            And(Atom("a"), Atom("b")),
            And(Atom("c"), Atom("d")),
            And(Atom("e"), Atom("f")),
...
def _contains_type(formula, target_type) -> bool:
    """Check whether a formula contains a node of the given type."""
...
def _has_double_negation(formula) -> bool:
    """Check whether a formula has a double negation."""
...
def _has_negation_over_compound(formula) -> bool:
    """Check whether there is a Not directly over And or Or."""
    if isinstance(formula, Not):
...
def _is_literal(formula) -> bool:
    """Check whether it is a literal (atom or negation of an atom)."""
...
def _is_clause(formula) -> bool:
    """Check whether it is a clause (disjunction of literals)."""
